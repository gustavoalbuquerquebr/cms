...
function generateAlert(status) {
  ...
  if (status) {
    message = `<strong>Success:</strong> Page <span class="font-italic"> ${page_title} </span> was deleted.`;
    style = "alert-success";
  } else {
    message = `<strong>Error:</strong> Page <span class="font-italic"> ${page_title} </span> wasn't deleted. Try again!`;
    style = "alert-danger";
  }
...
tbody.addEventListener("click", function(e) {
  if (e.target.classList.contains("delete-link")) {
    ...
    page_id = e.target.parentElement.dataset.id;
    page_title = row.querySelectorAll("td")[3].textContent;
  }
});
